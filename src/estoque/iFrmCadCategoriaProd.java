 ...
 * @author patri
 */
public class iFrmCadCategoriaProd extends javax.swing.JInternalFrame {
    private JDBQuery qryCatProd;
     ...
     */
    public iFrmCadCategoriaProd() {
        qryCatProd = new JDBQuery();
        qryCatProd.setJDBConnection(principal.Principal.conexao.getConexao());        
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Cadastro de Categorias de Produto");
...
//        int[] tamanho = {200, 400};
        ...
        pesquisa.iFrmPesquisa catProd;
        catProd = new pesquisa.iFrmPesquisa(this,
                "Pesquisa de Categorias de produtos",
                principal.Principal.conexao.getConexao(),
                qryCatProd,
                "id",
                combo,
                titulo,
                "SELECT id, descricao, data_inclusao, data_modificacao FROM categorias_produto",
                invisivel,
                null);

        
        principal.FrmPrincipal.Desktop.add(catProd);
    ...
    private void btnSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSalvarActionPerformed
        if(FrmLogin.usuario.verificarPermissao(this, 1)){
            ...
            if(txtDesc.getText().isEmpty()){
                JOptionPane.showMessageDialog(iFrmCadCategoriaProd.this, 
                        "Informe uma descrição", "Atenção", 
                        JOptionPane.INFORMATION_MESSAGE);
                txtDesc.requestFocus();
            }else if(txtDesc.getText().length() > 100){
                JOptionPane.showMessageDialog(iFrmCadCategoriaProd.this, 
                        "Descrição deve ter no máximo 100 caracteres", "Atenção", 
                        JOptionPane.INFORMATION_MESSAGE);
                txtDesc.requestFocus();
            }else{
                String sql;
                if(alteracao){
                    if(sistema.Mensagem.showConfirmDialog(iFrmCadCategoriaProd.this,
                        "Deseja realmente alterar esta categoria de produto?", "Confirmação") == 0){
        ...
        if(FrmLogin.usuario.verificarPermissao(this, 2)){
            ...
            if(alteracao){
                if(sistema.Mensagem.showConfirmDialog(this,
                        "Deseja realmente excluir esta categoria de produto?", "Confirmação") == 0){

                    String sql = String.format("DELETE FROM categorias_produto WHERE id = '%s'", id);
                ...
                JOptionPane.showMessageDialog(this, 
                        "Consulte uma categoria de produto para excluir", "Atenção", 
                        JOptionPane.INFORMATION_MESSAGE);
    ...
    public void atribuirCampos(){
        ...
        id = qryCatProd.getCurrentFieldValueAsString("id");
        descricao = qryCatProd.getCurrentFieldValueAsString("descricao");
        inclusao = qryCatProd.getCurrentFieldValueAsString("data_inclusao");
        modificacao = qryCatProd.getCurrentFieldValueAsString("data_modificacao");
    ...
    private boolean enviarTransacao(String sql){
        ...
        if(!concluido){
            ...
            if(transacao.getErrorCode() == 1062){
                JOptionPane.showMessageDialog(this, 
                        "Categoria de produto já cadastrada", "Atenção", 
                        JOptionPane.INFORMATION_MESSAGE);
